...
class LanguageAwareTextSplitter:
    """
    Text splitter that handles Tamil and English text appropriately
    """
    
    def __init__(self):
        # Tamil sentence separators (including Tamil punctuation)
        self.tamil_separators = [
            "\n\n",  # Paragraph breaks
            "\n",    # Line breaks
            "।",     # Devanagari danda (sometimes used in Tamil)
            ".",     # Period
            "!",     # Exclamation
            "?",     # Question mark
            ";",     # Semicolon
        ...
        # English separators (existing pattern)
        ...
        logger.info("Initialized Language-Aware Text Splitter")
    
    def split_text(self, text: str, language: Literal['english', 'tamil']) -> List[str]:
        """
        Split text based on language-specific patterns
        """
        if language == 'tamil':
            separators = self.tamil_separators
            logger.info("Using Tamil text splitting patterns")
        else:
        ...
        try:
            ...
            logger.info(f"Split {language} text into {len(chunks)} chunks")
